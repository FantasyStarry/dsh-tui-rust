/**
 * Vendored from MoonshotAI/kimi-code `packages/pi-tui/src/keys.ts`
 * (MIT License, Copyright (c) 2026 Moonshot AI), which itself vendors
 * earendil-works/pi-mono `packages/tui` (MIT License, Copyright (c) 2025
 * Mario Zechner). Only the license header and two `exactOptionalPropertyTypes`
 * adaptations (`?: number | undefined`) differ from upstream; matching logic
 * is verbatim so future diffs against upstream stay trivial.
 *
 * Orca 用法：`Keyboard`（见 `./input.js`）用 `decodePrintableKey` 把 Kitty
 * CSI-u / xterm modifyOtherKeys 的可打印序列还原成文本（`disambiguate`
 * 模式下终端会把裸 `a` 发成 `\x1b[97;1u`），用 `isKeyRelease` 吞掉 flag 2
 * 的 release 事件。`matchesKey` / `parseKey` / `Key` 暂作键绑定匹配的统一
 * 出口（当前 `classify` 仍沿用 readline 风格命名，保持不动）。
 */
...
interface ParsedKittySequence {
  codepoint: number;
  shiftedKey?: number | undefined; // Shifted version of the key (when shift is pressed)
  baseLayoutKey?: number | undefined; // Key in standard PC-101 layout (for non-Latin layouts)
  modifier: number;
...
function parseKittySequence(data: string): ParsedKittySequence | null {
  ...
  if (csiUMatch) {
    ...
    const shiftedKey = csiUMatch[2] != null && csiUMatch[2].length > 0 ? parseInt(csiUMatch[2], 10) : undefined;
    const baseLayoutKey = csiUMatch[3] != null ? parseInt(csiUMatch[3], 10) : undefined;
    const modValue = csiUMatch[4] != null ? parseInt(csiUMatch[4], 10) : 1;
  ...
  if (arrowMatch) {
    ...
    const arrowCode = arrowCodes[arrowMatch[3] ?? ''];
    if (arrowCode === undefined) return null;
    return { codepoint: arrowCode, modifier: modValue - 1, eventType };
  }
  ...
  if (funcMatch) {
    ...
    const modValue = funcMatch[2] != null ? parseInt(funcMatch[2], 10) : 1;
...
export function matchesKey(data: string, keyId: KeyId): boolean {
  ...
  if (key.length === 1 && ((key >= 'a' && key <= 'z') || isDigitKey(key) || SYMBOL_KEYS.has(key))) {
    ...
    if (modifier === MODIFIERS.ctrl + MODIFIERS.alt && !_kittyProtocolActive && rawCtrl != null) {
    ...
    if (modifier === MODIFIERS.ctrl) {
      ...
      if (rawCtrl != null && data === rawCtrl) return true;
      return (
 ...
 */
function formatParsedKey(codepoint: number, modifier: number, baseLayoutKey?: number | undefined): string | undefined {
...
export function parseKey(data: string): string | undefined {
  ...
  if (legacySequenceKeyId != null) return legacySequenceKeyId;
...
export function decodeKittyPrintable(data: string): string | undefined {
  ...
  const shiftedKey = match[2] != null && match[2].length > 0 ? Number.parseInt(match[2], 10) : undefined;
  const modValue = match[4] != null ? Number.parseInt(match[4], 10) : 1;
