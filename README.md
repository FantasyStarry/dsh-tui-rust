...
| `/ask <问题>` | 向 agent 提问，本轮只回答、不执行工具；也支持 agent 通过 `dsh-tool-ask-user` 发起问题（需 profile 挂载该工具），TUI 以 picker 展示选项，支持多选（Space）与自定义回答 |
