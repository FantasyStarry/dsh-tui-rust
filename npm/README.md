...
# 3. 发布（包名 @fantasystarry/dsh-tui —— 无 scope 的 dsh-tui 已被他人占用；
#    publishConfig.access=public 已写入 package.json，无需再传 --access）
cd npm
npm publish
```
...
- 无 scope 的 `dsh-tui` 包名已被他人占用（MashedPotato817/dsh-tui，与本仓库
  无关），故发布名固定为 **`@fantasystarry/dsh-tui`**；改回无 scope 名会继续
  E404。
